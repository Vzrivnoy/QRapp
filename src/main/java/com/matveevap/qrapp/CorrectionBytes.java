...
public class CorrectionBytes {
    public static byte[][] getCorrectionBytes(byte[][] dataChunks, ServiceInfo serviceInfo) {
        int numberOfBlocks = serviceInfo.numberOfBlocks();
        int numberOfCorrectionBytes = serviceInfo.numberOfCorrectionBytes();
        int[] generatingPolynomial = serviceInfo.generatingPolynomial();
        byte[][] chunksOfCorrectionBytes = new byte[numberOfBlocks][];

        for (int i = 0; i < numberOfBlocks; i++) {
            chunksOfCorrectionBytes[i] = getCorrectionBytesForChunk(dataChunks[i], numberOfCorrectionBytes, generatingPolynomial);
        }
        return chunksOfCorrectionBytes;
    }

    private static byte[] getCorrectionBytesForChunk(byte[] chunk, int numberOfCorrectionBytes, int[] generatingPolynomial) {
        int a, b, c;
        int numberOfBytesInBlock = chunk.length;
        ...
        for (int i = 0; i < matrix.length; i++) {
            if (i + 1 <= chunk.length) matrix[i] = chunk[i];
            else matrix[i] = 0;
        }
